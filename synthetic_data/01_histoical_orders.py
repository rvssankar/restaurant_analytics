...
MENU_BY_RESTAURANT=df_menu_items.groupby('restaurant_id').apply(lambda x:x.to_dict('records'),include_groups=False).to_dict()

print(MENU_BY_RESTAURANT)
...
def generate_historical_record(order_date):
...
def generate_historical_orders(num_orders=8000, months_back =6):
    ...
    for i in range(num_orders):
        ...
        order_date = order_date.replace(hour=random.randint(10,22),minute=random.randint(0,59),second=random.randint(0,59))

        order = generate_historical_record(order_date)
