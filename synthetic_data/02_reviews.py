...
def extract_items_from_orders(items_json):
...
def format_dishes(dishes_list):
    "Format dishes names for review text"
...
def generate_customer_reviews(review_percentage =0.35):
    ...
    for rating,weights in rating_weights.items():
        ratings_pool.extend([rating]*int(weights*100))
    ...
    for idx,order in df_orders.iterrows():
        ...
        dishes=extract_items_from_orders(order['items'])
